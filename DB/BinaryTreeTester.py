...
def printTree(btree, node, parent, relationship):
	# if ref is None:
		# return
	# node = getNodeFromRef(ref)
	if node is None:
		return
	key = "%s" % node.key
	value = "%s" % getNodeFromRef(btree, node.value_ref)
	print('%s of Parent Key: %s \t Key: %s, Value: %s' % (relationship, parent, key, value))
	printTree(btree, node.left, key, "left")
	printTree(btree, node.right, key, "right")
