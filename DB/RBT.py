...
new_tree = EmptyRedBlackTree().insert(0)
assert isinstance(new_tree, RedBlackTree)
new_tree = new_tree.insert(1)
new_tree = new_tree.insert(2)
new_tree = new_tree.insert(3)
new_tree = new_tree.insert(4)
new_tree = new_tree.insert(5)

def printTree(node, parent, relationship):
    if node is None or isinstance(node, EmptyRedBlackTree):
        return
    key = node.value
    print('%s of Parent Key: %s \t Key: %s' % (relationship, parent, key))
    printTree(node.left, key, "left")
    printTree(node.right, key, "right")

printTree(new_tree, 'none', 'none')
