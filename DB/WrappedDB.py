...
class WrappedDB:
	...
	def _keyIsInTopCached(self, key):
		topk_keys = sorted(self.keyToCount, key=keyToCount.get, reverse=True)[:self.cacheSize]
	...
	def _refreshCache(self, key, timeSeries):
		topk_keys = sorted(self.keyToCount, key=keyToCount.get, reverse=True)[:self.cacheSize]
		currentKeyCount = self.keyToCount[key] if key in self.keyToCount else 0

		if len(topk_keys) < self.cacheSize:
			# There are less keys in cache than cache size, just store
			self.cache[key] = timeSeries
		
		elif currentKeyCount >= self.keyToCount[topk_keys[:-1]]:
			# This time series now has a higher key count than the last of the top k
			lastKeyOfCache = topk_keys[:-1]
			del self.cache[lastKeyOfCache]
			self.cache[key] = timeSeries
			# Make sure that the cache size is correct
			assert len(self.cache) == self.cacheSize, "Size of cache is correct"

		# Update key count
		self.keyToCount[key] = currentKeyCount + 1
